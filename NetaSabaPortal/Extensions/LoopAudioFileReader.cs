...
namespace NetaSabaPortal.Extensions.NAudio
{
    /// <summary>
    ...
    /// </summary>
    public class LoopAudioFileReader : WaveStream, ISampleProvider
    {
        ...
        public bool MyProperty { get { bool r = false; return r; } }
        private bool ShouldLoop 
        { 
            get 
            {
                if (Loop < 0 || PlayedTimes < Loop) return true;
                return false;
            } 
        }
        public int Loop { get; set; } = 0;
        public int PlayedTimes { get; private set; }

        /// <summary>
        /// Initializes a new instance of AudioFileReader
        /// </summary>
        /// <param name="fileName">The file to open</param>
        public LoopAudioFileReader(string fileName, int loopTimes = 0)
        {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            lock (lockObject)  // 使用 lock 保護多執行緒訪問
            {
                while (totalSamplesRead < samplesRequired)
                {
                    int samplesRead = sampleChannel.Read(waveBuffer.FloatBuffer, (offset + totalSamplesRead) / 4, samplesRequired - totalSamplesRead);

                    if (samplesRead == 0)
                    {
                        PlayedTimes++;
                        // 如果未啟用循環或已到檔案結尾，跳出循環
                        if (!ShouldLoop || readerStream.Position == 0)
                        {
                            break;
                        }
